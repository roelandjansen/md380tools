...
void create_menu_entry_rbeep_enable_screen(void) {
  ...
  menu_mem = (menu_memory + ((*menu_depth) * sizeof(struct MENU))) + sizeof(struct MENU);
  menu_mem->menu_titel = wt_rbeep;

  menu_mem->unknownp = 0x14 * (*menu_id) + menu_mem_base;
...
void create_menu_entry_rbeep_screen(void) {
  ...
  for(i=0;i<2;i++) { // not yet known ;)
    uint8_t *p;
    p = menu_mem_base + ( (*menu_id) + i ) * 0x14;
    p[0x10] = 0;
  }
}
...
void m(void) {
  printf("your enter: ");
  printhex2((char *) menu_unkonwn_24,14);
  printf("\n");

  *menu_id    = *menu_id -1;
  *menu_depth = *menu_depth - 1;
  create_menu_entry_hook( (*menu_id), md380_menu_edit_buf,    menu_entry_back,  menu_entry_back  ,6, 1 , 1);
...
void create_menu_entry_edit_screen(void) {
  ...
  *menu_0x2001d3f4 = 0;
  *menu_0x20001114 = (uint32_t ) md380_menu_edit_buf;
...
// clear retrun buffer
  ...
  *menu_0x2001d3f1 = 0;
  ...
  create_menu_entry_hook( (*menu_id),  wt_edit ,  m + 1 , md380_menu_numerical_input  + 1,  0x81, 0 , 1);
}
